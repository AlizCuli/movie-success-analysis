...
def plot_feature_associations(
    ...
    grid = figure.add_gridspec(1, 2, width_ratios=[1.52, 0.78], wspace=0.28)
    ...
    axis_correlation.set_xticks(
        range(len(correlation.columns)), labels=correlation.columns, rotation=48, ha="right"
    )
    axis_correlation.set_yticks(range(len(correlation.index)), labels=correlation.index)
    axis_correlation.set_title("A. Tương quan Spearman giữa các biến số", loc="left", pad=12)
    ...
    for row in range(correlation.shape[0]):
        for column in range(correlation.shape[1]):
            ...
            axis_correlation.text(
                column, row, f"{value:.2f}", ha="center", va="center", color=color, fontsize=7.7
            )
    colorbar = figure.colorbar(image, ax=axis_correlation, fraction=0.035, pad=0.025)
    colorbar.set_label("Hệ số tương quan Spearman ρ")
    ...
    axis_groups.set_xlabel("Tỷ lệ is_successful (%)")
    ...
    axis_groups.tick_params(axis="y", length=0)
    axis_groups.legend(frameon=False, loc="lower right")
    ...
    figure.text(
        0.06,
        0.945,
        "Spearman ρ cho biến số; tỷ lệ is_successful theo primary_genre và is_collection "
        f"· n = {movie_count:,} phim".replace(",", "."),
        ha="left",
        fontsize=10.5,
        color="#4B5563",
    )
    figure.text(
        0.06,
        0.025,
        "Các kết quả chỉ phản ánh mối liên hệ trong mẫu nghiên cứu, không chứng minh quan hệ "
        "nhân quả. Biểu đồ B hiển thị 8 primary_genre có nhiều phim nhất.",
        ha="left",
        fontsize=9.5,
        color="#4B5563",
    )
    figure.subplots_adjust(top=0.86, bottom=0.15, left=0.06, right=0.97)
...
def run_eda() -> dict[str, object]:
    ...
    figures = [
        ...
        plot_feature_associations(correlation, genre_collection, len(modeling)),
    ]
